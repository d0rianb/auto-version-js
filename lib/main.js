...
class AutoVersion {
    ...
    static getPackageJSON(pathname) {
        const pkgPath = path.resolve(pkgDir.sync(pathname) || AutoVersion.getLocalPath(), 'package.json')
     ...
     * @param {string} [pathname]         the path of the package.json
     * @param {number} [indentation]      the number of space to pretty print the package.json file (auto-detected if omitted)
     * @example
    ...
    static setVersion(version, pathname, indentation) {
        const pkgRoot = pkgDir.sync(pathname) || AutoVersion.getLocalPath()
        const pkgPath = path.resolve(pkgRoot, 'package.json')
        const packageJSON = JSON.parse(fs.readFileSync(pkgPath, 'utf-8'))
        const indent = indentation || AutoVersion.detectIndentation(pkgPath)
...
const AutoGit = require('./git.js')

module.exports = AutoVersion
module.exports.AutoGit = AutoGit
