...
class AutoVersion {
    ...
    static getPackageJSON(pathname) {
        const packageJSON = require(path.resolve(pathname || AutoVersion.getPath(), 'package.json')) // store the package.json file as JSON
        return packageJSON
    }
    ...
    static setVersion(version, indentation) {
        const packageJSON = AutoVersion.getPackageJSON()
        packageJSON.version = version
        fs.writeFileSync(path.resolve(AutoVersion.getPath(), 'package.json'), JSON.stringify(packageJSON, null, indentation || 4))
    }
