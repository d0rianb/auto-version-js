...
let packageJSON  // store the package.json file as JSON
...
class AutoVersion {
     ...
     */
    static getVersion(dirname) {
        console.log(pkgDir.sync(dirname))
        packageJSON = require(path.resolve(dirname || pkgDir.sync(__dirname), 'package.json'))
        return packageJSON.version
    }

    /**
     * Write the version number into package.json
     * @param {string} dirname            the directory containing the package.json file
     * @param {string} version            the version number
     * @param {number} [indentation=4]    the number of space to pretty print the package.json file
     */
    static setVersion(dirname, version, indentation) {
        packageJSON.version = version
        fs.writeFileSync(`${dirname}/package.json`, JSON.stringify(packageJSON, null, indentation || 4))
    }
